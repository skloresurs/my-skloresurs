...
export default function UserSettingsPage({ params }: { params: { id: string } }) {
  ...
  if (isValidating)
    return (
      <>
        <TitleBar title="Керування користувачем" enableBackButton={true} />
  ...
  return (
    <>
      <TitleBar title={`Керування користувачем ${user?.fullname}`} enableBackButton={true} />
      <Tabs value={activeTab} onChange={(key) => router.push(`/admin/users/${user?.id}?tab=${key}`)}>
        <ScrollArea type="never">
          <Tabs.List className="flex-nowrap">
            <Tabs.Tab value="orders" leftSection={<Boxes size={20} />} disabled={true}>
              Замовлення
            </Tabs.Tab>
