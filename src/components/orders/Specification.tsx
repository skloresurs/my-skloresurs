...
export default function Specification({
  ...
  useEffect(() => {
    if (activeTab === 'new') {
      ...
      form.removeListItem(
        'specification',
        form.values.specification.length - 1
      );
      if (type?.twoGrops) {
        form.removeListItem(
          'specification',
          form.values.specification.length - 2
        );
      }
      setActiveTab('0');
    }
    ...
    <Fieldset legend="Параметри і склад виробу" className="relative space-y-2">
      {!form.values.type && (
        <>
          <Title
            order={3}
            className="absolute left-1/2 top-1/2 z-[9999] -translate-x-1/2 -translate-y-1/2"
          >
            Виберіть тип
          </Title>
        ...
        <Tabs.List>
          {form.values.specification?.map((e, i) => (
            <Tabs.Tab
              ...
              value={i.toString()}
              className={
                Object.keys(form.errors).findIndex((ee) =>
                  ee.startsWith(`specification.${i}.`)
                ) === -1
                  ? ''
                  : 'text-red-500'
              }
          ...
          <Tabs.Tab
            value="remove"
            disabled={form.values.specification?.length < 2}
          >
            -
          </Tabs.Tab>
          <Tabs.Tab
            value="new"
            disabled={
              type?.maxElementGroups
                ? type?.maxElementGroups <= form.values.specification?.length
                : true
            }
          >
            +
      ...
      <div className="space-y-2">
        <Select
          ...
          data={elementTypes.map((e) => ({
            ...e,
            disabled:
              !e.avalibleFor?.includes(form.values.type) && !!e.avalibleFor,
          }))}
          allowDeselect={false}
        />
        ...
        <Switch
          label="Гідрофоб"
          className="mt-2"
          {...form.getInputProps(`specification.${activeTab}.hydrofob`)}
        />
      </div>
    </Fieldset>
  );
