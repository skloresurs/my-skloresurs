...
export default function OperationSelect({ label, activeTab, form, formKey, operationId, className }: IProps) {
  const { data, isValidating, error } = useSWR(`${env.NEXT_PUBLIC_API_URL_1C_MAIN}/data/operations/${operationId}`);
  return (
    <Select
      className={className}
      label={label}
