    ...
    <>
      ...
      >
        ...
        <div className='mt-3 flex w-full flex-col gap-1 text-[var(--mantine-color-gray-5)]'>
          ...
          <div className='flex flex-row items-center gap-1'>
            <MapPin />
            <span className='line-clamp-2 w-full text-left'>{order.location.trim()}</span>
          </div>
        </div>
        {user?.permissions.includes('ManagerFinance') && (
          <div className='flex flex-row justify-end'>
            <NumberFormatter className='text-lg' value={order.total} suffix={` ${order.currency}`} decimalScale={2} />
          </div>
        )}
      </Paper>
