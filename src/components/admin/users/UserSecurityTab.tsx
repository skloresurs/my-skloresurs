...
export default function UserSecurityTab({ user }: { user?: IUserRequest }) {
  ...
  return (
    <form
      onSubmit={form.onSubmit(updateIps)}
      ...
      <Paper withBorder shadow="md" radius="md" className="py-3">
        <div className="px-4">
          ...
          <span className="text-sm">
            Список дозволених IP адрес для входу в систему
          </span>
          ...
          <div className="flex flex-col gap-1">
            {form.values.ips.map((ip, i) => (
              <div key={ip.key} className="flex flex-row items-center gap-2">
                <TextInput
                  {...form.getInputProps(`ips.${i}.ip`)}
                  className="w-full"
                />
                <ActionIcon
                  color="red"
                  variant="subtle"
                  onClick={() => form.removeListItem('ips', i)}
                >
                  <XCircle />
                </ActionIcon>
              </div>
          ...
          <div className="mt-2 flex flex-row items-center justify-end">
            <Button
              onClick={() =>
                form.insertListItem('ips', { ip: '', key: nanoid() })
              }
            >
              Додати
            </Button>
        ...
        <div className="flex flex-row items-center justify-between px-4">
          <span className="text-sm text-[var(--mantine-color-gray-5)]">
            Це не вплине на активні сесії!
          </span>
          <Button onClick={updateIps} loading={loading}>
            Зберегти
          </Button>
        </div>
      </Paper>
