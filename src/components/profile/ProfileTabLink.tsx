    ...
    <Paper withBorder shadow="md" radius="md" className="py-3">
      <div className="px-4">
        ...
        <span className="text-sm">
          Входьте в свій аккаунт за допомогою облікового запису {label}
        </span>
        <TextInput
          className="mt-3"
          value={value ?? valueId ?? "Не з'єднано"}
          readOnly
        />
      </div>
        ...
        <Button
          onClick={() =>
            window.location.replace(
              valueId ? `/api/profile/unlink/${id}` : `/api/auth/${id}`
            )
          }
